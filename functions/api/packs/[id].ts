...
export const onRequestGet: PagesFunction<Env> = async ({ params, env, request }) => {
  ...
  if (!pack.is_public && !isOwner) return json({ error: "Not found." }, { status: 404 })
...
export const onRequestPatch: PagesFunction<Env> = async ({ params, env, request }) => {
  ...
  const id = parseInt(String(params.id), 10)
  const pack = await loadPack(env, id)
  if (!pack || pack.user_id !== user.uid) return json({ error: "Not found." }, { status: 404 })
...
export const onRequestDelete: PagesFunction<Env> = async ({ params, env, request }) => {
  ...
  if (!user) return json({ error: "Sign in." }, { status: 401 })
  const id = parseInt(String(params.id), 10)
  const res = await env.DB.prepare("DELETE FROM packs WHERE id = ?1 AND user_id = ?2").bind(id, user.uid).run()
